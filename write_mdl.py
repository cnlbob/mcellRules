...
def construct_nfsim_mdl(jsonPath, mdlrPath, outputFileName, nautyDict):
    ...
    final_mdl.write('\n')
    final_mdl.write(
        'INCLUDE_FILE = "{0}.molecules.mdl"\n'.format(outputFileName))
    final_mdl.write(
        'INCLUDE_FILE = "{0}.reactions.mdl"\n'.format(outputFileName))
    ...
    molecule_mdl.write(
        '\t{0} //{1}\n\t{{ \n'.format(
            'volume_proxy',
            'proxy molecule type. the instance contains the actual '
            'information'))
    ...
    molecule_mdl.write(
        '\t{0} //{1}\n\t{{ \n'.format(
            'surface_proxy',
            'proxy surface type. the instance contains the actual '
            'information'))
    molecule_mdl.write('\t\tDIFFUSION_CONSTANT_{0}D = {1}\n'.format(2, 1))
    molecule_mdl.write('\t\tEXTERN\n')
    molecule_mdl.write('\t}\n')
    ...
    for seed in jsonDict['rel_list']:
        seed_mdl.write('\t{0} RELEASE_SITE //bng:{1}\n\t{{\n'.format(
            seed['name'], bngLabel[seed['molecule']]))

        # add scene qualifier to geometries
        shapeDescription = seed['object_expr']
        shapeDescription = [
            'Scene.' + x.strip() for x in shapeDescription.split(' - ')]
        shapeDescription = ' - '.join(shapeDescription)
        seed_mdl.write('\t\tSHAPE = {0}\n'.format(shapeDescription))
        ...
        seed_mdl.write(
            '\t\tGRAPH_PATTERN = "{0}"\n'.format(
                nautyDict[bngLabel[seed['molecule']]]))
        seed_mdl.write('\t}\n')
    seed_mdl.write('}\n')
